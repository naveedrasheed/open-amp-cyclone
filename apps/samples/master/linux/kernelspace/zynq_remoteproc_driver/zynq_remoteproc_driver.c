/*
 * cyclone Remote Processor driver
 *
...
{
	struct cyclone_rproc_pdata *local = platform_get_drvdata(remoteprocdev);
...
static int cyclone_rproc_start(struct rproc *rproc)
{
	struct device *dev = rproc->dev.parent;
	struct platform_device *pdev = to_platform_device(dev);
	struct cyclone_rproc_pdata *local = platform_get_drvdata(pdev);
	int ret;
	...
	remoteprocdev = pdev;
	ret = cyclone_cpun_start(0, 1);

	return ret;
}

/* kick a firmware */
static void cyclone_rproc_kick(struct rproc *rproc, int vqid)
{
	struct device *dev = rproc->dev.parent;
	struct platform_device *pdev = to_platform_device(dev);
	struct cyclone_rproc_pdata *local = platform_get_drvdata(pdev);
...
static int cyclone_rproc_stop(struct rproc *rproc)
{
...
static struct rproc_ops cyclone_rproc_ops = {
	.start	  = cyclone_rproc_start,
	.stop	   = cyclone_rproc_stop,
	.kick	   = cyclone_rproc_kick,
};

/* Just to detect bug if interrupt forwarding is broken */
static irqreturn_t cyclone_remoteproc_interrupt(int irq, void *dev_id)
{
...
static int cyclone_remoteproc_probe(struct platform_device *pdev)
{
	...
	struct irq_list *tmp;
	int count;
	struct cyclone_rproc_pdata *local;
	...
	local = kzalloc(sizeof(struct cyclone_rproc_pdata), GFP_KERNEL);
	if (!local) {
	...
	while (count--) {
		 ...
		 * use these IRQs */
		ret = request_irq(tmp->irq, cyclone_remoteproc_interrupt, 0,
	...
	if (prop) {
		...
		local->rproc = rproc_alloc(&pdev->dev, dev_name(&pdev->dev),
				&cyclone_rproc_ops, prop, sizeof(struct rproc));
		if (!local->rproc) {
...
static int cyclone_remoteproc_remove(struct platform_device *pdev)
{
	struct cyclone_rproc_pdata *local = platform_get_drvdata(pdev);
	u32 ret;
...
static struct of_device_id cyclone_remoteproc_match[] = {
	{ .compatible = "xlnx,cyclone_remoteproc", },
	{ /* end of list */ },
};
MODULE_DEVICE_TABLE(of, cyclone_remoteproc_match);

static struct platform_driver cyclone_remoteproc_driver = {
	.probe = cyclone_remoteproc_probe,
	.remove = cyclone_remoteproc_remove,
	.driver = {
		.name = "cyclone_remoteproc",
		.owner = THIS_MODULE,
		.of_match_table = cyclone_remoteproc_match,
	},
};
module_platform_driver(cyclone_remoteproc_driver);
...
MODULE_DESCRIPTION("cyclone remote processor control driver");
